        ...
        Parser parser = new Parser() {
            ...
            public Block parse(List<Token> input) throws SyntaxException, Task3Exception {
                if(input.get(0) instanceof T_LeftCurlyBracket) {
                    ...
                    expressions.add((Exp) new BlockExp(parse(input)));
                    ...
                    if(input.get(0) instanceof T_Integer) {
                        input.remove(0);                        
                        expressions.add((Exp) new IntLiteral(((T_Integer)t).n));
                        parse(input);
                    } else if(input.get(0) instanceof T_Skip) {
                        input.remove(0);
                        expressions.add((Exp) new Skip());
                        parse(input);
...
{
    public String msg;    
    public Task3Exception(String _msg) {
        msg = _msg;
    }
}
