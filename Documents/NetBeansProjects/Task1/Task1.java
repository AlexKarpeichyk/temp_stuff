...
class CodegenClass implements Codegen
{
    ...
    public static void main(String[] args) throws CodegenException {
        ...
        Declaration d = new Declaration("f", 3, new If(new Variable(1), new Equals(), new Variable(2), new Variable(3), new IntLiteral(0)));
        List<Declaration> progs = new ArrayList<>();
        progs.add(d);
        Program p = new Program(progs);
        System.out.println((cc.codegen(p)));
    }
    */
    @Override
    public String codegen(Program p) throws CodegenException {
        if(p.decls.size() == 1) {
            codegenDeclaration(p.decls.get(0));
        } else if(p.decls.size() > 1){
            codegen(p);
        } else {
    ...
    public void codegenDeclaration(Declaration d) {
        mips_prog += "\n" + ".data";
        codegenVardec(d.numOfArgs);
    ...
    public void codegenVardec(int num) {
        mips_prog = mips_prog + "\n\t" + "dest" + (dest_increment++) + ": .space " + num;
    }
    
    public void codegenExp(Exp e) {
    ...
    private void codegenInvoke(List<Exp> args, String name) {
        mips_prog = mips_prog + "\n\t" + "sw $fp 0($sp)"
                              + "\n\t" + "addiu $sp $sp -4";
        if(args.size() == 0) {
            mips_prog = mips_prog + "\n\t" + "jal " + name;
